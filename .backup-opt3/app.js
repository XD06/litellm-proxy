(() => {
  ...
  function renderTrafficComboChart(options) {
    ...
    const yFor = (value, max) => height - pad.bottom - (Number(value || 0) / Math.max(1, Number(max || 1))) * plotH;
    const yToken = (value) => yFor(value, tokenMax);
    const yRequest = (value) => yFor(value, requestMax);
    const baseline = yToken(0);
    const enriched = buckets.map((bucket, index) => ({
      ...bucket,
      x: xFor(bucket, index, buckets.length),
    }));
    const lineConfigs = [
      { key: "total_tokens", label: "Total tokens", className: "traffic-total-line", dotClass: "traffic-total-dot", axis: "token", format: fmtTokenCount },
      { key: "input", label: "Input", className: "traffic-input-line", dotClass: "traffic-input-dot", axis: "token", format: fmtTokenCount },
      { key: "output", label: "Output", className: "traffic-output-line", dotClass: "traffic-output-dot", axis: "token", format: fmtTokenCount },
      { key: "requests", label: "Requests", className: "traffic-request-line", dotClass: "traffic-request-dot", axis: "request", format: fmtInt },
      { key: "failed", label: "Failures", className: "traffic-failure-line", dotClass: "traffic-failure-dot", axis: "request", format: fmtInt },
    ];
    const pointsFor = (config) => enriched.map((bucket) => {
      const value = Number(bucket[config.key] || 0);
      return {
        ...bucket,
        value,
        y: config.axis === "token" ? yToken(value) : yRequest(value),
      };
    });
    const buildPath = (config) => {
      const points = pointsFor(config);
      if (!points.some((point) => Number(point.value || 0) > 0)) return "";
      return smoothSvgPath(points, pad.top, baseline);
    };
    const totalPoints = pointsFor(lineConfigs[0]);
    const totalPath = buildPath(lineConfigs[0]);
    const areaPath = totalPath && totalPoints.length > 1 && totalPoints.some((point) => point.value > 0)
      ? `${totalPath} L ${svgNum(totalPoints[totalPoints.length - 1].x)} ${svgNum(baseline)} L ${svgNum(totalPoints[0].x)} ${svgNum(baseline)} Z`
      : "";
    const seriesPaths = lineConfigs.map((config) => {
      const path = config.key === "total_tokens" ? totalPath : buildPath(config);
    ...
    const seriesDots = lineConfigs.map((config) => {
      const points = pointsFor(config);
      if (points.length > 64) return "";
      return points
        .filter((point) => Number(point.value || 0) > 0)
        .map((point) => `
          <circle class="traffic-trend-dot ${config.dotClass}" cx="${svgNum(point.x)}" cy="${svgNum(point.y)}" r="${config.key === "total_tokens" ? "4" : "3.2"}">
    ...
    const legend = lineConfigs.map((config) => `
      <span class="traffic-trend-legend-item ${config.dotClass}">
        <i></i>${escapeHtml(config.label)}
      </span>
    `).join("");

    return `
      <div class="traffic-chart-shell">
        <svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Usage trend chart with token and request lines">
          <defs>
            <linearGradient id="trafficTokenArea" x1="0" x2="0" y1="0" y2="1">
              <stop offset="0%" stop-color="#a855f7" stop-opacity="0.2"></stop>
              <stop offset="45%" stop-color="#a855f7" stop-opacity="0.075"></stop>
          ...
          ${requestLabels.map((label) => `
            <text class="traffic-axis-label traffic-axis-label-info" x="${width - pad.right + 14}" y="${yRequest(label) + 4}">${escapeHtml(fmtInt(label))}</text>
          `).join("")}
          ${xTicks.map((point) => `
            <line class="axis traffic-x-tick" x1="${svgNum(point.x)}" y1="${pad.top}" x2="${svgNum(point.x)}" y2="${height - pad.bottom}"></line>
            <text class="traffic-axis-label" x="${svgNum(point.x)}" y="${height - 30}" text-anchor="middle">${escapeHtml(shortDate(point.start || point.ts))}</text>
          `).join("")}
          <line class="axis traffic-y-axis" x1="${pad.left}" y1="${pad.top}" x2="${pad.left}" y2="${baseline}"></line>
          <line class="axis traffic-y-axis traffic-y-axis-right" x1="${width - pad.right}" y1="${pad.top}" x2="${width - pad.right}" y2="${baseline}"></line>
          <line class="axis traffic-baseline" x1="${pad.left}" y1="${baseline}" x2="${width - pad.right}" y2="${baseline}"></line>
          ${tokenLabels.map((label) => `
            <line class="axis traffic-axis-tick" x1="${pad.left - 5}" y1="${yToken(label)}" x2="${pad.left}" y2="${yToken(label)}"></line>
          `).join("")}
          ${requestLabels.map((label) => `
            <line class="axis traffic-axis-tick traffic-axis-tick-right" x1="${width - pad.right}" y1="${yRequest(label)}" x2="${width - pad.right + 5}" y2="${yRequest(label)}"></line>
          `).join("")}
          ${areaPath ? `<path class="traffic-token-area" d="${areaPath}"></path>` : ""}
          ${seriesPaths}
          ${seriesDots}
          <text class="traffic-axis-title" x="${pad.left}" y="${pad.top - 8}">tokens</text>
          <text class="traffic-axis-title traffic-axis-label-info" x="${width - pad.right}" y="${pad.top - 8}" text-anchor="end">requests</text>
        </svg>
  ...
  function renderRequestsTable() {
    ...
    if (!items.length) {
      target.innerHTML = `<div class="request-list-head">${requestPagination(total, currentPage, totalPages, items.length)}</div><div class="empty pad">No matching requests</div>`;
      bindRequestPagination(target, totalPages);
      return;
    }
    const rows = items.map(requestSummaryRow).join("");
    target.innerHTML = `
      <div class="request-list-head">${requestPagination(total, currentPage, totalPages, items.length)}</div>
  ...
  function requestSummaryRow(r) {
    ...
    return `
      <button class="request-summary-row tone-${escapeHtml(statusTone)}" type="button" data-request-id="${escapeHtml(r.request_id)}">
        <span class="request-row-dot"></span>
  ...
  function requestPagination(total, currentPage, totalPages, visibleCount) {
    const start = total ? state.requestsPage * REQUEST_PAGE_SIZE + 1 : 0;
      ...
      <article class="provider-runtime-card provider-health-tile ${view.runtimeState.tone}" data-provider-card="${escapeHtml(view.name)}" tabindex="0">
        <div class="provider-card-topline">
          ...
          <div class="provider-title-block">
            <div class="provider-name" title="${escapeHtml(view.name)}">${escapeHtml(view.name)}</div>
  ...
  function failurePolicyDescription(errorType) {
    ...
    if (text.includes("network")) return "Network and timeout failures can cool both key and provider to avoid hot-looping.";
