...
import json, os, uuid, datetime, sys, socket, concurrent.futures, time, re, threading, hmac
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _proxy_openai_responses(self, req, request_id, start_ts):
        is_stream = bool(req.get("stream", False))
        original_model = req.get("model", "")
        resolved_model = resolve_model(original_model or "")
        canonical_model = resolved_model
    ...
    def do_POST(self):
        ...
        if is_responses:
            return self._proxy_openai_responses(req, request_id, start_ts)
