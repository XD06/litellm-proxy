...
路由器的格式偏好由 `routing.format_preference` 控制，默认为 `priority_first`：
1. **priority_first（默认）**：供应商 `priority` 决定全局顺序；同格式（native）仅在相同优先级时作为 tie-breaker 优先于跨格式（fallback）供应商
2. **native_first（传统模式）**：所有 native 供应商排在所有 fallback 供应商之前，不受 priority 影响
3. 可通过 `models.routes.{model}.format_preference` 为单个模型覆盖全局策略
4. 避免在同一请求中重试相同的 `provider + key_index + upstream_format` 组合
...
| `network_error` | 超时/断开/连接失败 | key | 10s | ❌ |
...
"failure_policies": {
  "network_error": {
    "cooldown_scope": "key",  // none | key | key_provider（代码内置默认只冷却 key）
    "cooldown_s": 10,
...
| `ANTHROPIC_API_KEY` | 零配置自动检测 |
| `DEEPSEEK_API_KEY` | 零配置自动检测 |
| `GROQ_API_KEY` / `OPENROUTER_API_KEY` | 零配置自动检测 |
| `MISTRAL_API_KEY` / `TOGETHER_AI_API_KEY` / `FIREWORKS_API_KEY` | 零配置自动检测 |
| `PERPLEXITY_API_KEY` / `XAI_API_KEY` | 零配置自动检测 |
| `SILICONFLOW_API_KEY` / `MOONSHOT_API_KEY` / `ZHIPUAI_API_KEY` | 零配置自动检测 |
| `DASHSCOPE_API_KEY` / `VOLC_API_KEY` | 零配置自动检测 |

> 完整零配置预置列表见 [§15 零配置模式](#15-零配置模式)。
...
| 端点 | 方法 | 功能 |
|---|---|---|
| `/-/admin/status` | GET | 供应商状态、健康、密钥可用性、零配置预置信息 |
| `/-/admin/metrics` | GET | 轻量指标快照（计数器 + 失败摘要 + 活跃数） |
| `/-/admin/metrics/full` | GET | 完整指标快照（含 recent_requests 环形缓冲） |
| `/-/admin/metrics/timeseries` | GET | 时序指标桶 |
| `/-/admin/requests` | GET | 请求历史列表（支持筛选/分页） |
| `/-/admin/requests/{id}` | GET | 单条请求详情 |
| `/-/admin/requests/clear` | POST | 清空请求历史（需 `confirm: clear_request_history`） |
| `/-/admin/requests/delete` | POST | 删除指定请求记录（需 `confirm: delete_request_records`） |
| `/-/admin/requests/delete-matching` | POST | 按筛选条件批量删除请求记录 |
| `/-/admin/routing` | GET | 路由策略快照 |
| `/-/admin/routing` | PATCH | 修改路由配置 |
| `/-/admin/retry` | PATCH | 修改重试策略 |
| `/-/admin/retry/failure-policies` | PATCH | 修改单种错误类型的失败策略 |
| `/-/admin/config` | GET | 当前配置快照（脱敏） |
| `/-/admin/config` | POST | 重新加载基础配置文件 |
| `/-/admin/config/overlay` | GET | 查看运行时覆盖层 |
| `/-/admin/config/overlay/validate` | POST | 预览覆盖层合并结果（不保存） |
| `/-/admin/config/overlay/clear` | POST | 清除运行时覆盖层（需 `confirm: clear_runtime_overlay`） |
| `/-/admin/config/overlay/compact` | POST | 压缩覆盖层为持久化增量 |
| `/-/admin/providers` | POST | 新增供应商 |
| `/-/admin/providers/{name}` | PATCH | 修改供应商配置（模型映射/禁用模型等） |
...
| `/-/admin/providers/{name}/keys/{idx}/delete` | POST | 删除密钥 |
| `/-/admin/providers/{name}/keys/{idx}/test` | POST | 探测密钥可用性 |
| `/-/admin/providers/{name}/models/refresh` | POST | 刷新指定供应商的模型发现 |
| `/-/admin/models/refresh` | POST | 刷新全部模型发现 |
| `/-/admin/models/routes` | PATCH | 新增/修改模型路由 |
| `/-/admin/models/routes/delete` | POST | 删除模型路由 |
| `/-/admin/models/infer-mapping` | POST | 推断模型到供应商的映射 |
| `/-/admin/models/capabilities` | GET | 模型能力快照 |
...
| 环境变量 | 自动创建的供应商 | 上游格式 | Base URL | 优先级 |
|---|---|---|---|---|
| `OPENAI_API_KEY` | `openai` | chat_completions + responses | `https://api.openai.com` | 10 |
| `ANTHROPIC_API_KEY` | `anthropic` | anthropic_messages | `https://api.anthropic.com` | 10 |
| `DEEPSEEK_API_KEY` | `deepseek` | chat_completions | `https://api.deepseek.com` | 8 |
| `XAI_API_KEY` | `xai` | chat_completions | `https://api.x.ai` | 7 |
| `GROQ_API_KEY` | `groq` | chat_completions | `https://api.groq.com/openai` | 7 |
| `MISTRAL_API_KEY` | `mistral` | chat_completions | `https://api.mistral.ai` | 7 |
| `OPENROUTER_API_KEY` | `openrouter` | chat_completions | `https://openrouter.ai/api` | 6 |
| `TOGETHER_AI_API_KEY` | `together` | chat_completions | `https://api.together.xyz` | 6 |
| `FIREWORKS_API_KEY` | `fireworks` | chat_completions | `https://api.fireworks.ai/inference` | 6 |
| `SILICONFLOW_API_KEY` | `siliconflow` | chat_completions | `https://api.siliconflow.cn` | 6 |
| `MOONSHOT_API_KEY` | `moonshot` | chat_completions | `https://api.moonshot.cn` | 6 |
| `ZHIPUAI_API_KEY` | `zhipuai` | chat_completions | `https://open.bigmodel.cn/api/paas` | 6 |
| `DASHSCOPE_API_KEY` | `dashscope` | chat_completions | `https://dashscope.aliyuncs.com/compatible-mode` | 6 |
| `VOLC_API_KEY` | `volcengine` | chat_completions | `https://ark.cn-beijing.volces.com/api` | 6 |
| `PERPLEXITY_API_KEY` | `perplexity` | chat_completions | `https://api.perplexity.ai` | 5 |
...
| 连接池 | 代理核心使用 urllib3，无 HTTP/2 支持 |
