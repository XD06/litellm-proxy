...
14. Routing continuation hardening: completed on 2026-07-08. Non-fatal HTTP errors (`model_not_found` 404/400, `schema_or_client_error` 422, unrecognized non-retryable status codes) no longer set `stop_attempts=True`, so the attempt loop continues to the next provider instead of breaking. `max_attempts` is now clamped to at least `prov_count` (total provider+key candidates), ensuring every available provider gets a chance before the proxy gives up. Client-facing error messages no longer include internal provider/status/error_type details; the simplified message "All upstream providers are currently unavailable" is returned, while the detailed attempt log is printed to server logs and recorded in observability. See `docs/fix-plan-2026-07-08.md`.

15. New provider priority auto-assignment: completed on 2026-07-08. `config_manager.add_provider` now auto-assigns a priority lower than all existing providers (`min(existing) - 1`) when no explicit `priority` is set, preventing new providers from accidentally intercepting traffic from configured providers. The dashboard "Add Provider" form omits `priority` from the API payload when the field is 0, allowing the backend to auto-assign. See `docs/fix-plan-2026-07-08.md`.

16. Dashboard Add Provider clipboard auto-fill: completed on 2026-07-08. The "Add Provider" modal now includes a "Paste & Auto-fill" button that reads the clipboard and detects URL/API key patterns to auto-fill the `base_url` and `key` fields. Provider preset chips (Anthropic, DeepSeek, Groq, OpenRouter, xAI, Mistral, SiliconFlow, Moonshot, Together) provide one-click base URL and format filling. See `docs/fix-plan-2026-07-08.md`.

17. Frontend i18n priority direction corrections: completed on 2026-07-08. `form.priority_tip` and `pm.higher_first` Chinese translations were corrected from "lower number = higher priority" to "higher number = higher priority", matching the actual `priority_failover` sorting logic in `router.py`. See `docs/fix-plan-2026-07-08.md`.
