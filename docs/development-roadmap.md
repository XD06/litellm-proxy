...
- Automatic discovery writes `models.provider_model_capabilities` as an in-memory runtime snapshot.
...
- Model discovery first tries `provider.base_url + models_path`; if `base_url` includes a path and that fails, it tries the origin root plus `models_path`. This supports providers whose generation API is path-prefixed while models are listed at the root `/v1/models`.
