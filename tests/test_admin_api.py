...
class IdleProbeAdvancedTests(unittest.TestCase):
    ...
    def test_find_best_model_provider_pair_with_recent_model(self):
        """Test global model selection: finds recent model and picks highest priority provider that supports it."""
        cfg = {
            "models": {
                "provider_model_map": {
                    "alpha": {"shared-model": "alpha-shared"},
                    "beta": {"beta-only": "beta-only", "shared-model": "beta-shared"},
                },
            },
            "providers": {
                "alpha": {
        ...
        # Mock recent success: beta provider used 'shared-model' successfully
        with patch.object(obs, 'latest_successful_model_for_provider') as mock_latest:
            def side_effect(provider):
                if provider == "beta":
                    return "shared-model"  # This model should be found
                elif provider == "alpha":
                    return "alpha-only"  # This should be ignored
                return None
            mock_latest.side_effect = side_effect
            
            model, provider, source = sse2json._find_best_model_provider_pair(obs, cfg, router)
            
            # Should find the shared model and select alpha (higher priority) since it supports the model
            self.assertEqual(model, "shared-model")
            self.assertEqual(provider, "alpha")  # Alpha has higher priority and supports shared-model
            self.assertEqual(source, "recent_success_global")

    def test_find_best_model_provider_pair_no_model_support_fallback(self):
        """Test fallback when no provider supports the recent model."""
        cfg = {
            "models": {
                "provider_model_map": {
                    "alpha": {"alpha-only": "alpha-only"},
        ...
        # Mock recent success: beta provider used 'unsupported-model' that no provider supports
        with patch.object(obs, 'latest_successful_model_for_provider') as mock_latest:
            def side_effect(provider):
                if provider == "beta":
                    return "unsupported-model"  # This model is not in any provider's config
                return "some-other-model"  # Different model for other providers
            mock_latest.side_effect = side_effect
            
            model, provider, source = sse2json._find_best_model_provider_pair(obs, cfg, router)
            
            # Alpha has higher priority but no provider supports 'unsupported-model'
            # Should fall back to alpha with alpha-only model
            self.assertEqual(provider, "alpha")  # Highest priority provider
            self.assertEqual(model, "alpha-only")  # Alpha's own model
            self.assertTrue(source.startswith("fallback_no_model_support_"))

    def test_find_best_model_provider_pair_no_recent_model(self):
        """Test when there's no recent model, should use highest priority provider with any model."""
        cfg = {
            "models": {
                "provider_model_map": {
        ...
        # No recent successful models
        with patch.object(obs, 'latest_successful_model_for_provider') as mock_latest:
            mock_latest.return_value = None
            
            model, provider, source = sse2json._find_best_model_provider_pair(obs, cfg, router)
            
            self.assertEqual(model, "alpha-model")
