	...
	*/
    ...
    `;
		...
		function providerModelItems(name, capability) {
			const items = modelCapabilityItemsMemo(name, Array.isArray(capability.models) ? capability.models : [], capability.canonical_map || {}).slice();
    ...
    `;
		...
		function formatRouteItems(formats, provider) {
			...
			return rows.map(([name, cfg]) => {
				...
				const dataAttrs = interactive ? `data-format-provider="${escapeHtml(provider)}" data-format="${escapeHtml(name)}" data-format-enabled="${enabled ? "1" : "0"}" data-format-path="${escapeHtml(cfg?.path || "")}" role="button" tabindex="0"` : "";
